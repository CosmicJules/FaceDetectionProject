...
for stage  = 1:5
    ShowHaarCasade(HaarCasade,stage)
end
...
subplot(1,2,1)
imagesc(IntegralImages.ii);
subplot(1,2,2)
imagesc(IntegralImages.ii2);
...
% Threshold is how much area of a given box must be intersected before we
% consider any NMS e.g. 0.5 for 50%

% Rectangle is of form X1,Y1,W,H,,COLOUR,CONF
returnBoxes = givenBoxes;
toDelete = [];

for k=1:size(givenBoxes,1)
    curRect = givenBoxes(k,:);
    % May need to add 1 for adjustment, may also have no effect
    ...
    for j=1:size(givenBoxes,1)
        ...
        % If they intersect (and we are not looking at same rectangle)
        if k ~= j && curIntersect > 0
            % Delete based on condifdence
